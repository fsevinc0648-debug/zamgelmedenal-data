...
RSS2JSON = "https://api.rss2json.com/v1/api.json?rss_url={}&count=12"
...
def rss_cek(sorgu: str):
    # Yabancı (İngilizce) edisyon: içerik İngilizce gelir, AI Türkçeye çevirip yeniden yazar
    url = "https://news.google.com/rss/search?q={}&hl=en-US&gl=US&ceid=US:en".format(quote(sorgu))
    r = requests.get(RSS2JSON.format(quote(url)), timeout=25)
    j = r.json()
    if j.get("status") != "ok":
        return []
    out = []
    for it in j.get("items", []):
        ...
        if p > 10:
            ...
            t = t[:p].strip()
        out.append({
            "ham_baslik": temizle(t),
            "ham_ozet": temizle(it.get("description", ""))[:400],
